...
// Model for Movie entity
    models.Movie = Backbone.Model.extend({
        urlRoot: "http://localhost:50551/Cinemopolis-web/webresources/movie/",
        idAttribute: 'movieId',
        defaults: {
            actors: "",
            movie: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('movieId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:50551/Cinemopolis-web/webresources/movie/';
            }
    ...
    // Collection class for Movie entities
    models.MovieCollection = Backbone.Collection.extend({
        model: models.Movie,
        url: "http://localhost:50551/Cinemopolis-web/webresources/movie/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MovieCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-movie-list-itemis template identifier for item
                        templateName: '#tpl-movie-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.movie = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.movie,
                    // tpl-movie-details is template identifier for chosen model element
                    templateName: '#tpl-movie-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Movie();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-movie-details is a template identifier for chosen model element
                templateName: '#tpl-movie-details',
